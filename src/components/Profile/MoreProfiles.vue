  ...
  <div class="profile-container" v-if="!loadingProfiles">
    <div v-for="(profile, index) in otherUsers" :key="profile._id">
      <v-col class="profilesCol" outlined>
        <v-card
          class="mx-auto theCard"
          :style="{ background: colors[index], 'box-shadow': boxShadow[index] }"
        >
          ...
          <v-card-actions>
            <v-btn @click="getProfile(profile.username)" class="view-button"
              >view</v-btn
            >
...
export default {
  ...
  data() {
    return {
      ...
      boxShadow: [
        "0px 0px 10px rgba(9, 155, 94, 0.986);",
        "0px 0px 10px rgba(245, 20, 245, 0.986)",
        "0px 0px 10px rgba(224, 177, 8, 0.986)",
